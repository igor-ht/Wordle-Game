...
export async function validateUser(req: Request, res: Response, next: NextFunction) {
	await check('name').isLength({ min: 2, max: 50 }).withMessage('Your name must contain between 2 and 50 characters.').run(req),
		await check('email').isEmail().withMessage('Email not valid.').run(req),
...
export async function validadeUserUpdate(req: Request, res: Response, next: NextFunction) {
	await check('id').isInt({ gt: 0, lt: 101 }).withMessage('ID not valid.').run(req),
		...
		await check('newUser.name').isLength({ min: 2, max: 50 }).withMessage('Your name must be between 2 and 50 characters.').run(req),
